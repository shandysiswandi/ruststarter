...
use app_orm::prelude::{MfaFactors, Roles, UserConnections, UserCredentials, UserRoles, Users};
use app_orm::{mfa_factors, roles, user_connections, user_credentials, user_roles, users};
...
use crate::domain::entity::user::{User, UserStatus, UserUpdatePayload};
...
impl AuthRepository for AuthORM {
    ...
    async fn find_user_connections_by_user_id(&self, user_id: i64) -> Result<Vec<UserConnection>, AppError> {
        ...
        let output = connections.into_iter().map(|model| self.to_user_connection(model)).collect();

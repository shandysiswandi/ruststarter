...
use crate::domain::entity::user::{NewUser, User, UserUpdatePayload};
...
pub trait AuthRepository: Send + Sync {
    ...
    ///
    /// Users are ordered by their creation date in descending order (newest
    /// first).
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// * `Ok((Vec<User>, u64))` - A tuple containing the list of users for the
    ///   requested page and the total number of matching users.
    /// * `Err(AppError)` - If the query fails due to a database or internal
    ///   error.
    ...
    /// * `Err(AppError)` if the user could not be updated (e.g., not found or
    ///   DB error).
    ...
    /// # Arguments
    ///
    /// * `uid` - The unique ID of the user whose credentials should be
    ///   retrieved.
    ///
    /// # Returns
    ///
    ...
    /// and verification in a single transaction.
    ///
    /// # Arguments
    ///
    ///  `user` - A `NewUser` struct containing registration data.
    ///
    /// # Returns
    ///
    /// * `Ok(())` if both user and credential were created successfully.
    /// * `Err(AppError)` If the operation failed at any stage.
    async fn create_new_user(&self, user: &NewUser) -> Result<(), AppError>;

    /// Updates the stored credential (password) for a given user.
    ///
    /// # Arguments
    ///
    /// * `user_id` - The unique ID of the user whose credential is being
    ///   updated.
    /// * `new_password` - The new password to store (will typically be hashed
    ///   before persistence).
    ///
    /// # Returns
    ///
    ...
    /// Retrieves a user verification entry based on the provided verification
    /// token.
    ///
    /// # Arguments
    ///
    /// * `token` - The unique verification token associated with a pending user
    ///   verification.
    ///
    /// # Returns
    ///
    /// * `Ok(UserVerification)` if a matching verification entry is found.
    /// * `Err(AppError)` if no entry exists or a database query/mapping error
    ///   occurs.
    async fn find_user_verification_by_token(&self, token: &str) -> Result<Option<UserVerification>, AppError>;

    /// Marks a user as verified and removes the corresponding verification
    /// token in a single database transaction.
    ///
    /// # Arguments
    ///
    /// * `user_id` - The unique ID of the user being verified.
    /// * `token` - The unique verification token associated with the user.
    ///
    /// # Returns
    ///
    /// * `Ok(())` if the user was successfully marked as verified and the
    ///   verification token was deleted.
    /// * `Err(AppError)` if the user does not exist, the token does not exist,
    ///   or if a database error occurs.
    async fn verify_user_and_delete_token(&self, user_id: i64, token: &str) -> Result<(), AppError>;

    /// Retrieves all linked third-party authentication connections for a given
    /// user.
    ///
    /// # Arguments
    ///
    /// * `user_id` - The unique ID of the user whose connections are being
    ///   fetched.
    ///
    /// # Returns
    ///
    /// * `Ok(Vec<UserConnection>)` containing zero or more connections if
    ///   successful.
    ...
    /// # Arguments
    ///
    /// * `payload` - A `OAuthUserProfile` entity containing the provider,
    ///   provider user ID, and any associated authentication tokens or scopes.
    ///
    /// # Returns
    ///
    /// * `Ok(User)` - The user associated with the connection, whether found or
    ///   newly created.
    ...
    ///
    /// This is typically used to remove a linked third-party account (e.g.,
    /// Google, GitHub) from an existing user profile.
    ///
    /// # Arguments
    ///
    /// * `user_id` - The unique ID of the user who owns the OAuth connection.
    /// * `connection_id` - The unique ID of the OAuth connection record to
    ///   delete.
    ///
    /// # Returns
    ///
    /// * `Ok(())` - If the connection was successfully deleted.
    /// * `Err(AppError)` - If the delete operation failed (e.g., connection not
    ///   found, DB error).
    async fn delete_oauth_user(&self, user_id: i64, connection_id: i64) -> Result<(), AppError>;

    /// Retrieves all registered multi-factor authentication (MFA) factors for a
    /// specific user.
    ///
    /// This method queries the database for all MFA factor records associated
    /// with the given user ID. MFA factors can include items such as TOTP
    /// authenticators, security keys, or backup codes.
    ///
    /// # Arguments
    ///
    /// * `user_id` - The unique ID of the user whose MFA factors will be
    ///   retrieved.
    ///
    /// # Returns
    ///
    ...
    /// Retrieves a specific multi-factor authentication (MFA) factor by ID and
    /// User ID.
    ///
    /// This is typically used during MFA enrollment when a factor has been
    /// created but not confirmed (e.g., before a TOTP secret or security
    /// key registration is validated).
    ///
    /// # Arguments
    ///
    ...
    /// # Returns
    ///
    /// * `Ok(Some(MfaFactor))` - If the unverified MFA factor exists and
    ///   belongs to the user.
    ...
    /// Creates a new multi-factor authentication (MFA) factor record for a
    /// given user.
    ///
    ...
    /// The `MfaFactor` domain object should contain all required details such
    /// as the associated user ID, factor type, secret or credential data,
    /// and a friendly display name.
    ///
    /// # Arguments
    ///
    /// * `factor` - A fully populated `MfaFactor` domain object representing
    ///   the MFA factor to store.
    ///
    /// # Returns
    ///
    /// * `Ok(())` - If the MFA factor was successfully created.
    /// * `Err(AppError)` - If the creation fails due to a database or internal
    ///   error.
    ...
    ///
    /// This is typically called after a user successfully completes the MFA
    /// setup process (e.g., entering a correct TOTP code for the first
    /// time), indicating that the factor is now active and can be used for
    /// authentication.
    ///
    /// # Arguments
    ///
    ...
    /// Deletes a specific multi-factor authentication (MFA) factor for a given
    /// user.
    ///
    /// This is typically used when a user disables an MFA method such as a TOTP
    /// authenticator, security key, or backup code.
    ///
    /// # Arguments
    ///
    /// * `user_id` - The unique ID of the user who owns the MFA factor.
    ...
    /// * `Err(AppError)` - If the delete operation failed (e.g., factor not
    ///   found, DB error).
    async fn delete_mfa_factor_by_id(&self, user_id: i64, factor_id: i64) -> Result<(), AppError>;

    /// Sets the roles for a specific user.
    ///
    /// This method replaces any existing roles assigned to the user with the
    /// provided list. Typically used in administrative contexts to manage
    /// user permissions.
    ///
    /// # Arguments
    ///
    /// * `user_id` - The unique identifier of the user whose roles are being
    ///   updated.
    /// * `role_ids` - A list of role IDs to assign to the user. Existing roles
    ///   will be replaced.
    ///
    /// # Returns
    ///
    ...
    ///
    /// This method verifies that every role ID in the provided slice
    /// corresponds to an existing role in the system.
    ///
    /// # Arguments
    ///

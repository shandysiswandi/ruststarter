...
impl ProfileService {
    pub fn new(
        ...
        Self { config, uid, storage, hasher, repo }
    }
...
impl ProfileUseCase for ProfileService {
    ...
    async fn setup_totp(&self, input: SetupTotpInput) -> Result<SetupTotpOutput, AppError> {
        ...
        let qr_code_image = totp.get_qr_base64().map_err(AppError::TotpQrGeneration)?;

        tracing::info!("TOTP setup initiated for user: {} with factor: {}", input.user_id, factor_id);

        Ok(SetupTotpOutput { factor_id, secret: secret.to_string(), qr_code_image })
    }
